...
class SequenceSpace(Space):
    ...
    def __hash__(self):
        ...
        .. todo::
            ...
            WRITEME
        """
        return hash((type(self), self.sub_space))

    @functools.wraps(Space.get_origin)
    def get_origin(self):
        return [self.sub_space.get_origin()]#TODO 
    ...
    @functools.wraps(Space._format_as_impl)
    def _format_as_impl(self, is_numeric, batch, space):
        #TODO:Sequence(IndexSapce)---vectorSpace  Sequence(VectorSpace)---Conv2DSapce
        #Sequence(k tensor)----(k+1) tensor
        to_type = None

    @functools.wraps(Space._undo_format_as_impl)
    def _undo_format_as_impl(self, is_numeric,  batch, space):
        pass
    ...
    def _validate_impl(self, is_numeric, batch):
        """
        #is_numric=0
        batch.dtype=object and batch[i].dtype=object/int/ depends

        #is_numric=1
        for batch.dtype=object, so complete calidate is to check sub_space._validate_impl(is_numric=1,batch[i][k]) for all
            WRITEME
        """
        super(IndexSequenceSpace, self)._validate_impl(is_numeric, batch)
        if is_numetic:
